...
static void tcp_fin(struct sock *sk)
{
	 ...
	 */
	skb_rbtree_purge_sk(sk, &tp->out_of_order_queue);
	if (tcp_is_sack(tp))
...
restart:
	...
	while (before(start, end)) {
		...
		if (!nskb)
			break;
		if (ub_tcprcvbuf_charge_forced(sk, nskb) < 0) {
			kfree_skb(nskb);
			break;
		}
...
static int tcp_prune_ofo_queue(struct sock *sk)
{
	...
	do {
		...
		skb = rb_entry(node, struct sk_buff, rbnode);
		goal -= skb->truesize;
		skb->sk = sk;
		__kfree_skb(skb);
		if (!prev || goal <= 0) {
...
EXPORT_SYMBOL(tcp_rbtree_insert);
