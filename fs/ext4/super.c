...
static int ext4_show_options(struct seq_file *seq, struct vfsmount *vfs)
{
	...
	if (sbi->s_ve_safe_ino)
		seq_printf(seq, ",ve_safe_ino=%ld", sbi->s_ve_safe_ino->i_ino);
...
enum {
	...
	Opt_discard, Opt_nodiscard, Opt_balloon_ino, Opt_ve_safe_ino,
...
static void ext4_load_ve_safe(struct super_block *sb, unsigned long ino)
{
	struct inode *inode;
	struct inode *old_ino = NULL;
	struct ext4_sb_info *sbi;
	...
	if (!ino) {
		spin_lock(&sbi->s_ve_safe_lock);
		if (sbi->s_ve_safe_ino) {
			old_ino = sbi->s_ve_safe_ino;
			sbi->s_ve_safe_ino = NULL;
		}
		spin_unlock(&sbi->s_ve_safe_lock);
		iput(old_ino);
		return;
	...
	if (ino < EXT4_FIRST_INO(sb)) {
		ext4_msg(sb, KERN_WARNING, "bad ve_safe inode specified");
		return;
	}

	inode = ext4_iget(sb, ino);
	if (IS_ERR(inode)) {
		ext4_msg(sb, KERN_WARNING, "can't load ve_safe inode (%ld)", PTR_ERR(inode));
		return;
	}

	if (!S_ISDIR(inode->i_mode)) {
		iput(inode);
		ext4_msg(sb, KERN_WARNING, "ve_safe should be directory");
		return;
	}
	...
	if (sbi->s_ve_safe_ino)
		old_ino = sbi->s_ve_safe_ino;
	sbi->s_ve_safe_ino = inode;
	spin_unlock(&sbi->s_ve_safe_lock);
	iput(old_ino);

	ext4_msg(sb, KERN_INFO, "loaded ve_safe from %ld",
			inode->i_ino);
}
				...
				__acquires(kernel_lock)
{
	...
	if (!parse_options((char *) sbi->s_es->s_mount_opts, sb,
			   &journal_devnum, &journal_ioprio, &balloon_ino, &ve_safe_ino, NULL, 0)) {
		ext4_msg(sb, KERN_WARNING,
			 "failed to parse options in superblock: %s",
			 sbi->s_es->s_mount_opts);
	}
	if (!parse_options((char *) data, sb, &journal_devnum,
			   &journal_ioprio, &balloon_ino, &ve_safe_ino, NULL, 0))
		goto failed_mount;
...
no_journal:
	...
	spin_lock_init(&sbi->s_ve_safe_lock);
	ext4_load_ve_safe(sb, ve_safe_ino);
...
static void ext4_kill_sb(struct super_block *sb)
{
	...
	if (sbi && sbi->s_ve_safe_ino)
		iput(sbi->s_ve_safe_ino);
