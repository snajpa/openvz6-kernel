...
void device_shutdown(void)
{
	...
	spin_lock(&ve_devices_kset->list_lock);
	/*
	...
	while (!list_empty(&ve_devices_kset->list)) {
		dev = list_entry(ve_devices_kset->list.prev, struct device,
				kobj.entry);
		...
		spin_unlock(&ve_devices_kset->list_lock);
