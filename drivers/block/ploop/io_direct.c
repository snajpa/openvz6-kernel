	   ...
	   struct bio_list *sbl, iblock_t iblk, unsigned int size)
{
	...
	if ((rw & BIO_FUA) && ploop_req_delay_fua_possible(preq)) {
		/* Mark req that delayed flush required */
		preq->req_rw |= (BIO_FLUSH | BIO_FUA);
		delayed_fua = 1;
	}
...
flush_bio:
		...
		bio->bi_rw |= bw.cur->bi_rw &
			(BIO_FLUSH | delayed_fua ? 0 : BIO_FUA);
		bw.bv_off += copy;
		size -= copy >> 9;
		sec += copy >> 9;
	}
...
flush_bio:
		...
		if (!zero_page)
			bio->bi_rw |= bw.cur->bi_rw & BIO_FLUSH;
	...
	while (bl.head) {
		...
		rw = preq->req_rw & ~(BIO_FLUSH | BIO_FUA);
	       ...
	       struct page * page, sector_t sec, unsigned long rw)
{
	...
	dio_io_page(io, WRITE | (1 << BIO_RW_SYNCIO), preq, page, sec);
}
...
static void dio_trim_prealloc(struct ploop_io * io, struct file * file)
{
	...
	if (size < i_size_read(io->files.inode)) {
		...
		trim_extent_mappings(io->plo, io->files.em_tree, size >> 9);
	}
}
